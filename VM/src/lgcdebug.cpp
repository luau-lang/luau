...
void validateobjref(global_State* g, GCObject* f, GCObject* t)
{
...
void validateref(global_State* g, GCObject* f, TValue* v)
{
    if (iscollectable(v))
    {
...
void validatetable(global_State* g, LuaTable* h)
{
...
void validateclosure(global_State* g, Closure* cl)
{
...
void validatestack(global_State* g, lua_State* l)
{
...
void validateproto(global_State* g, Proto* f)
{
    if (f->source)
...
void validateobj(global_State* g, GCObject* o)
{
...
void validategraylist(global_State* g, GCObject* o)
{
    if (!keepinvariant(g))
        return;
...
bool validategco(void* context, lua_Page* page, GCObject* gco)
{
...
void dumpref(FILE* f, GCObject* o)
{
    fprintf(f, "\"%p\"", o);
}

void dumprefs(FILE* f, TValue* data, size_t size)
{
    bool first = true;
...
void dumpstringdata(FILE* f, const char* data, size_t len)
{
    for (size_t i = 0; i < len; ++i)
        fputc(safejson(data[i]) ? data[i] : '?', f);
}

void dumpstring(FILE* f, TString* ts)
{
...
void dumptable(FILE* f, LuaTable* h)
{
...
void dumpclosure(FILE* f, Closure* cl)
{
    fprintf(
...
void dumpthread(FILE* f, lua_State* th)
{
...
void dumpupval(FILE* f, UpVal* uv)
{
...
void dumpobj(FILE* f, GCObject* o)
{
    switch (o->gch.tt)
    {
    case LUA_TSTRING:
...
bool dumpgco(void* context, lua_Page* page, GCObject* gco)
{
    FILE* f = (FILE*)context;
...
void* enumtopointer(GCObject* gco)
{
...
void enumnode(EnumContext* ctx, GCObject* gco, size_t size, const char* objname)
{
    ctx->node(ctx->context, enumtopointer(gco), gco->gch.tt, gco->gch.memcat, size, objname);
}

void enumedge(EnumContext* ctx, GCObject* from, GCObject* to, const char* edgename)
{
    ctx->edge(ctx->context, enumtopointer(from), enumtopointer(to), edgename);
}

void enumedges(EnumContext* ctx, GCObject* from, TValue* data, size_t size, const char* edgename)
{
    for (size_t i = 0; i < size; ++i)
    {
        if (iscollectable(&data[i]))
...
void enumstring(EnumContext* ctx, TString* ts)
{
    enumnode(ctx, obj2gco(ts), ts->len, NULL);
}

void enumtable(EnumContext* ctx, LuaTable* h)
{
    size_t size = sizeof(LuaTable) + (h->node == &luaH_dummynode ? 0 : sizenode(h) * sizeof(LuaNode)) + h->sizearray * sizeof(TValue);
...
void enumclosure(EnumContext* ctx, Closure* cl)
{
    if (cl->isC)
    {
...
void enumudata(EnumContext* ctx, Udata* u)
{
...
void enumthread(EnumContext* ctx, lua_State* th)
{
    size_t size = sizeof(lua_State) + sizeof(TValue) * th->stacksize + sizeof(CallInfo) * th->size_ci;
...
void enumbuffer(EnumContext* ctx, Buffer* b)
{
    enumnode(ctx, obj2gco(b), sizebuffer(b->len), NULL);
}

void enumproto(EnumContext* ctx, Proto* p)
{
    size_t size = sizeof(Proto) + sizeof(Instruction) * p->sizecode + sizeof(Proto*) * p->sizep + sizeof(TValue) * p->sizek + p->sizelineinfo +
                  sizeof(LocVar) * p->sizelocvars + sizeof(TString*) * p->sizeupvalues;
...
void enumupval(EnumContext* ctx, UpVal* uv)
{
...
void enumobj(EnumContext* ctx, GCObject* o)
{
    switch (o->gch.tt)
    {
    case LUA_TSTRING:
...
bool enumgco(void* context, lua_Page* page, GCObject* gco)
{
