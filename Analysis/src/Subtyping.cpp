...
SubtypingResult& SubtypingResult::orElse(const SubtypingResult& other)
{
    ...
    else if (FFlag::LuauUnifyWithSubtyping && other.isSubtype)
    {
        ...
        // the other check.
        //
        // It would also be nice to `std::move` this.
        assumedConstraints = other.assumedConstraints;
    }
...
SubtypingResult Subtyping::isCovariantWith(SubtypingEnvironment& env, TypeId subTy, TypeId superTy, NotNull<Scope> scope)
{
    ...
    if (FFlag::LuauUnifyWithSubtyping && get2<FreeType, FreeType>(subTy, superTy))
    {
    ...
    else if (auto superFree = get<FreeType>(superTy); FFlag::LuauUnifyWithSubtyping && superFree)
    {
    ...
    else if (auto subFree = get<FreeType>(subTy); FFlag::LuauUnifyWithSubtyping && subFree)
    {
    ...
    else if (FFlag::LuauUnifyWithSubtyping && (is<BlockedType>(subTy) || is<BlockedType>(superTy)))
    {
        result = {true};
        result.assumedConstraints.emplace_back(SubtypeConstraint{subTy, superTy});
    }
    else if (auto subGeneric = get<GenericType>(subTy); FFlag::LuauUnifyWithSubtyping && subGeneric && subsumes(subGeneric->scope, scope))
        return isCovariantWith(env, builtinTypes->neverType, superTy, scope);
    else if (auto superGeneric = get<GenericType>(superTy); FFlag::LuauUnifyWithSubtyping && superGeneric && subsumes(superGeneric->scope, scope))
        return isCovariantWith(env, subTy, builtinTypes->unknownType, scope);
    ...
    else if (auto pair = get2<FreeType, FreeType>(subTy, superTy); !FFlag::LuauUnifyWithSubtyping && pair)
    {
        // Any two free types are potentially subtypes of one another because
        // both of them could be narrowed to never.
        result = {true};
...
SubtypingResult Subtyping::isCovariantWith(SubtypingEnvironment& env, TypePackId subTp, TypePackId superTp, NotNull<Scope> scope)
{
    ...
    if (subTail && superTail)
    {
        ...
        else if (FFlag::LuauUnifyWithSubtyping && (is<FreeTypePack>(*subTail) || is<FreeTypePack>(*superTail)))
        {
            results.push_back(
    ...
    else if (subTail)
    {
        ...
        else if (FFlag::LuauUnifyWithSubtyping && is<FreeTypePack>(*subTail))
        {
    ...
    else if (superTail)
    {
        ...
        else if (FFlag::LuauUnifyWithSubtyping && is<FreeTypePack>(*superTail))
        {
            // This is the case where:
            // 1. Both the `superTp` and `subTp` have the same number of types in the head
...
)
{
    ...
    else if (FFlag::LuauUnifyWithSubtyping && get<FreeTypePack>(subTail))
    {
        TypePackId superTailPack = sliceTypePack(superHeadStartIndex, superTp, superHead, superTail, builtinTypes, arena);
        return SubtypingResult{true}.withSubComponent(TypePath::PackField::Tail).withAssumedConstraint(PackSubtypeConstraint{subTail, superTailPack});
    }
    else
        return SubtypingResult{false}
            .withSubComponent(TypePath::PackField::Tail)
...
SubtypingResult Subtyping::isCovariantWith(SubtypingEnvironment& env, TypeId subTy, const UnionType* superUnion, NotNull<Scope> scope)
{
    ...
    for (TypeId ty : superUnion)
    {
        ...
        if (next.isSubtype)
            return FFlag::LuauUnifyWithSubtyping ? next : SubtypingResult{true};
...
SubtypingResult Subtyping::isCovariantWith(SubtypingEnvironment& env, const TypeId subTy, const NegationType* superNegation, NotNull<Scope> scope)
{
    ...
    {
        // ¬any ~ any
        result = FFlag::LuauUnifyWithSubtyping ? isCovariantWith(env, subTy, negatedTy, scope) : isSubtype(subTy, negatedTy, scope);
    }
    else if (auto u = get<UnionType>(negatedTy))
    {
        // ¬(A ∪ B) ~ ¬A ∩ ¬B
...
)
{
    ...
    for (const auto& [name, superProp] : superTable->props)
    {
        ...
        else if (subTable->indexer)
        {
            if (isCovariantWith(env, builtinTypes->stringType, subTable->indexer->indexType, scope).isSubtype)
            {
                ...
                {
                    results.push_back(isInvariantWith(env, irt, *superProp.readTy, scope)
                ...
                {
                    if (superProp.readTy)
                        results.push_back(isCovariantWith(env, irt, *superProp.readTy, scope)
                                              .withSubComponent(TypePath::TypeField::IndexResult)
                                              .withSuperComponent(TypePath::Property::read(name)));
                    if (superProp.writeTy)
                        results.push_back(isContravariantWith(env, irt, *superProp.writeTy, scope)
                                              .withSubComponent(TypePath::TypeField::IndexResult)
    ...
    if (superTable->indexer)
    {
        if (subTable->indexer)
        {
            if (FFlag::LuauAnalysisReadWriteIndexers)
            {
                if ((superTable->indexer->readIndexResultType && !subTable->indexer->readIndexResultType)
                    || (superTable->indexer->writeIndexResultType && !subTable->indexer->writeIndexResultType)
                )
...
)
{
    if (FFlag::LuauAnalysisReadWriteIndexers)
    {
        SubtypingResult result = isInvariantWith(env, subIndexer.indexType, superIndexer.indexType, scope).withBothComponent(TypePath::TypeField::IndexLookup);
        if (!result.isSubtype)
            return result;

        if ((superIndexer.readIndexResultType && !subIndexer.readIndexResultType)
            || (superIndexer.writeIndexResultType && !subIndexer.writeIndexResultType)
        )
            return SubtypingResult{false}.withBothComponent(TypePath::TypeField::IndexLookup);

        if (superIndexer.readIndexResultType)
        {
            result = result.andAlso(
                isInvariantWith(env, subIndexer.readIndexResultType.value(), superIndexer.readIndexResultType.value(), scope).withBothComponent(TypePath::TypeField::IndexResult)
            );
            if (!result.isSubtype)
                return result;
        }

        if (superIndexer.writeIndexResultType)
            result = result.andAlso(
                isInvariantWith(env, subIndexer.writeIndexResultType.value(), superIndexer.writeIndexResultType.value(), scope).withBothComponent(TypePath::TypeField::IndexResult)
            );
    ...
    else
    {
        return isInvariantWith(env, subIndexer.indexType, superIndexer.indexType, scope)
            ...
            .andAlso(
                isInvariantWith(env, subIndexer.indexResultType_DEPRECATED, superIndexer.indexResultType_DEPRECATED, scope).withBothComponent(TypePath::TypeField::IndexResult)
            );
    }
}
