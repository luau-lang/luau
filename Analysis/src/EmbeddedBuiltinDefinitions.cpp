...
static constexpr const char* kBuiltinDefinitionBaseSrc_OldSolver = R"BUILTIN_SRC(
...
std::string getBuiltinDefinitionSource(SolverMode Solver)
{
    ...
    if (Solver == SolverMode::New && FFlag::LuauNewSolverNewDefinitions)
        result = kBuiltinDefinitionBaseSrc;
    else
        result = kBuiltinDefinitionBaseSrc_OldSolver;
