...
DefId DataFlowGraphBuilder::lookup(Symbol symbol, Location location)
{
    ...
    for (DfgScope* current = scope; current; current = current->parent)
    {
        ...
        else if (current->scopeType == DfgScope::Function)
        {
            ...
            // Refinements are keyed on `DefId`s, so every use of `symbol` within
            // this scope must resolve to the same capture phi, including uses
            // inside loops. The phi is populated later by `resolveCaptures`.
