...
stepᴱ H (M $ N) | value V p with stepᴱ H N
stepᴱ H (M $ N) | value V p | step H′ N′ D = step H′ (M $ N′) (app₂ p D)
stepᴱ H (nil $ _) | value nil refl | value W refl = error NilIsNotAFunction
stepᴱ H (addr a $ N) | value (addr a) refl | value W q with remember (H [ a ])
stepᴱ H (addr a $ N) | value (addr a) refl | value W q | (nothing , p) = error (app₁ (SEGV a p))
stepᴱ H (addr a $ N) | value (addr a) refl | value W q | (just(function F is B end) , p) = step H (block fun F is (B [ W / name(arg F) ]ᴮ) end) (beta p q)
