...
{
    Luau::DenseHashMap<int, std::string> m;
...
TEST_CASE("map_copy")
{
    ...
    Luau::DenseHashMap<int, int> copy(m);
...
TEST_CASE("map_move")
{
    ...
    Luau::DenseHashMap<int, int> moved(std::move(m));
...
{
    Luau::DenseHashMap<int, int> m1;
    for (int i = 0; i < 12; ++i)
        m1[i] = i;

    Luau::DenseHashMap<int, int> m2;
    for (int i = 8; i < 24; ++i)
        m2[i] = i;
...
{
    Luau::DenseHashMap<std::string, int> m;
...
TEST_CASE("set_move")
{
    ...
    Luau::DenseHashSet<int> moved(std::move(s));
...
{
    Luau::DenseHashSet<int> a;
    Luau::DenseHashSet<int> b;
...
TEST_CASE("set_erase_destroys_element_destructor")
{
    ...
    {
        Luau::DenseHashSet<std::shared_ptr<int>> s;
        s.insert(p);
        CHECK(p.use_count() == 2);
...
TEST_CASE("map_erase_destroys_value_destructor")
{
    ...
    {
        Luau::DenseHashMap<int, std::shared_ptr<int>> m;
        m[1] = p;
        CHECK(p.use_count() == 2);
...
{
    Luau::DenseHashMap<Luau::NotNull<int>, int> map;
    int x = 1;
