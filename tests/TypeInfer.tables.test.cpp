...
caused by:
  The given type's metatable does not satisfy the requirements.
required field 'absolutely_no_scalar_has_this_method' not found in type 'typeof(string)' from expected type 't1 where t1 = {+ absolutely_no_scalar_has_this_method: (t1) -> (T, U...) +}')";
        CHECK_EQ(expected, toString(result.errors[0]));
